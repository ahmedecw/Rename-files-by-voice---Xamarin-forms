...
namespace ImagePicker.Droid.DependencyServiceImplementation
{
    public class MediaServiceImplementation : IMediaService
    {
        ...
        public const int RequestCamera = 8765;
                    ...
                    if (cursor.Count > 0)
                    {
                        while (cursor.MoveToNext())
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(filePath) && path.Contains("DCIM")
  )
                                {
        ...
        public static Android.Net.Uri getImageUri(Context inContext, Bitmap inImage)
        {
            ...
            inImage = Bitmap.CreateScaledBitmap(inImage, 6120, 7160, false);
        ...
        private byte[] CreateImage(Bitmap bitmap, float width, float height)
        {
            ...
            //  ContentResolver.(MediaStore.Images.Media.ExternalContentUri, MediaStore.Images.Media.InterfaceConsts.Data + "=?", new string[] { TheImageURL });
            //   https://stackoverflow.com/questions/24564336/rename-an-image-with-mediastore-android
        }
        ...
        public string UpdateGallery(String filePath)
        {
